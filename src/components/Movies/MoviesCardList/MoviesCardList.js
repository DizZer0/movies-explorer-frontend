...
function MoviesCardList(props) {
  ...
  const [initialNumberCard, setInitialNumberCard] = React.useState()
  ...
  function getMovieList() {
    if(props.searchInputValue === '') {
      ...
      moviesApi.getMovies()
        .then(res => sortingMovieList(res))
    }
  }

  React.useEffect(() => {
    props.renderMoreBtn(movieList.length <= initialNumberCard)
  }, [initialNumberCard])
  ...
  return (
    <div className="card-list">
      {movieList.slice(0, initialNumberCard).map((card) => {
